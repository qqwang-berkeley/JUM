...
runtime=$((end-start))
echo "JUM_4.sh core execution finished in $runtime seconds."
echo "Command is:"
echo "bash JUM_4.sh --Folder $folder --Test $pvalue_padj --Cutoff $cutoff --condition1num $condition_num --condition2num $ctrl_num --REF $refflat"

if [ -d "temp_JUM_4_run" ]; then
rm -r temp_JUM_4_run;
fi

mkdir temp_JUM_4_run;
ls | grep -v temp_JUM_4_run | xargs mv -t temp_JUM_4_run;

#mv !(temp_JUM_4_run) temp_JUM_4_run/
mv temp_JUM_4_run/*detailed.txt .
mv temp_JUM_4_run/*simplified.txt .
