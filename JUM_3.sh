...
runtime=$((end-start))
echo "JUM_3.sh core execution finished in $runtime seconds."
echo "Command is:"
echo "bash JUM_3.sh --Folder $folder --Test $pvalue_padj --Cutoff $cutoff --Totalnum $file_num --Smallnum $con_file_num"


if [ -d "temp_JUM_3_run" ]; then
rm -r temp_JUM_3_run;
fi

mkdir temp_JUM_3_run;

#mv !(temp_JUM_3_run) temp_JUM_3_run/
ls | grep -v temp_JUM_3_run | xargs mv -t temp_JUM_3_run;
...
mv temp_JUM_3_run/AS_differential_JUM_output_*"$pvalue_padj"_"$cutoff".txt FINAL_JUM_OUTPUT_"$pvalue_padj"_"$cutoff";
mv temp_JUM_3_run/Valid_* FINAL_JUM_OUTPUT_"$pvalue_padj"_"$cutoff";
mv temp_JUM_3_run/total_mixed_event.txt FINAL_JUM_OUTPUT_"$pvalue_padj"_"$cutoff";
mv temp_JUM_3_run/total_intron_retention_event.txt FINAL_JUM_OUTPUT_"$pvalue_padj"_"$cutoff";
mv temp_JUM_3_run/*coordinate.bed FINAL_JUM_OUTPUT_"$pvalue_padj"_"$cutoff";

echo "JUM_3.sh completed. Ready for next step! Feel free to delete the temporary file folder temp_JUM_3_run once satisfied."
