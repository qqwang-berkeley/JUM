...
echo "JUM_2-3.sh core execution finished in $runtime seconds."
echo "Command is:"
echo "bash JUM_2-3.sh --Folder $folder --Threshold $threshold --Filenum $file_num --IRthreshold $IR_threshold --Readlength $read_length --Thread $thread_num"


if [ -d "temp_JUM_2-3_run" ]; then
rm -r temp_JUM_2-3_run;
fi

mkdir temp_JUM_2-3_run;

ls | grep -v temp_JUM_2-3_run | xargs mv -t temp_JUM_2-3_run;
...
mv temp_JUM_2-3_run/*combined_count.txt JUM_diff/
mv temp_JUM_2-3_run/combined_AS_JUM.gff JUM_diff/
mv temp_JUM_2-3_run/*Aligned.out_coverage.bed JUM_diff/
mv temp_JUM_2-3_run/*profiled_total_AS_event_junction_first_processing_for_JUM_reference_building.txt JUM_diff/
mv temp_JUM_2-3_run/UNION_junc_coor_with_junction_ID_more_than_"$threshold"_read_in_at_least_"$file_num"_samples.txt JUM_diff/

mv temp_JUM_2-3_run/*Aligned.out.sam .
mv temp_JUM_2-3_run/*Aligned.out_sorted.bam* .
mv temp_JUM_2-3_run/*formatted.txt .
mv temp_JUM_2-3_run/*SJ.out.tab .

echo "JUM_2-3.sh completed. Ready for next step! Feel free to delete the temporary file folder temp_JUM_2_run once satisfied."
