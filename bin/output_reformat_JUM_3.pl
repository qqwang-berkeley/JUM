...
#data_file_1=intron_long_sorted.txt; $data_file2=$condition_num; $data_file_3=$ctrl_num; 

my $len=scalar(@ARGV);
if ($len < 1) {
    die "Usage:output_reformat_3.pl <data_file_1> <data_file_2> <data_file_3>\n";
...
my @enter_3;
$#enter_3=$ctrl_num+$condition_num;
my @enter_4;
$#enter_4=$ctrl_num+$condition_num;
    ...
    if($_ !~ /raw_count/) {
      ...
      my @array=split(/\s+/,$_);
	   ...
	   print $ori; print "\n";
         ...
         if($count % 4 == 3) {
                 for($s=0; $s<=$ctrl_num+$condition_num-1; $s++) {
                 $enter_3[$s]=$array[15+$s];
                 }
	   print $ori; print "\n";
         }
         if($count % 4 == 0) {
                 for($s=0; $s<=$ctrl_num+$condition_num-1; $s++) {
                 $enter_4[$s]=$array[15+$s];
                 }
	 $indicator_1=0;
	 $indicator_2=0;
	 $sum1=0;
	 $sum2=0;
	     ...
	     if($enter_1[$s]+$enter_2[$s]+$enter_4[$s] != 0) {	
		    $ratio[$s]=($enter_2[$s]+$enter_4[$s])/($enter_1[$s]+$enter_2[$s]+$enter_4[$s]);
	     ...
	     if($enter_1[$m]+$enter_2[$m]+$enter_4[$m] != 0) {	
		    $ratio[$m]=($enter_2[$m]+$enter_4[$m])/($enter_1[$m]+$enter_2[$m]+$enter_4[$m]);
