...
use strict;
use warnings;
#data_file_1=UNION_junc_coor.txt; data_file_2=union_junc_coor_5_prime_ss_list_with_alternative_3_ss.txt; 

my $len=scalar(@ARGV);
if ($len < 1) {
    die "Usage:profile_AS_events_sharing_5_prime_ss.pl <data_file_1> <data_file_2>\n";
...
while(<IN1>) {
      chomp;
      my @temp=split(/\s+/,$_);
      if (exists $sscoor{$temp[0]}{$temp[1]}{$temp[2]}) {
           $sscoor{$temp[0]}{$temp[1]}{$temp[2]}[0]=$sscoor{$temp[0]}{$temp[1]}{$temp[2]}[0]+1;
           $sscoor{$temp[0]}{$temp[1]}{$temp[2]}[$sscoor{$temp[0]}{$temp[1]}{$temp[2]}[0]]=$temp[4];
           }
